  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter">Live Tracking</h2>
          <p className="text-white/70 text-xs font-medium uppercase tracking-widest">Monitoramento de frotas e coletas em tempo real</p>
        </div>
            ...
            >
              <div className="flex justify-between items-start relative z-10">
                <div className="flex gap-4">
                  <div className={cn(
                    "p-3 rounded-2xl",
                    item.status === 'em_rota' ? "bg-blue-100 text-blue-600" : 
                    item.status === 'atrasado' ? "bg-red-100 text-red-600" : "bg-gray-100 text-gray-400"
                  )}>
                    <User size={24} />
                  </div>
                  <div>
                    <h4 className={cn("font-black uppercase italic text-sm", selectedDriver.id === item.id ? "text-gray-800" : "text-white")}>
                        {item.motorista}
                    </h4>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{item.veiculo}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className={cn(
                    "text-[9px] font-black uppercase px-2 py-1 rounded-md",
                    item.status === 'em_rota' ? "bg-blue-500 text-white" : 
                    item.status === 'atrasado' ? "bg-red-500 text-white" : "bg-gray-700 text-white"
                  )}>
              ...
              {item.progresso > 0 && (
                <div className="mt-6">
                  <div className="flex justify-between text-[9px] font-black uppercase mb-1">
                    <span className={selectedDriver.id === item.id ? "text-gray-400" : "text-white/40"}>Progresso da Rota</span>
                    <span className={selectedDriver.id === item.id ? "text-orange-600" : "text-orange-400"}>{item.progresso}%</span>
                  </div>
        ...
        <div className="col-span-12 lg:col-span-7">
          <div className="bg-white rounded-[3rem] shadow-2xl p-8 h-full border border-white/10">
            <div className="flex justify-between items-start mb-10">
                <div>
                    <h3 className="text-2xl font-black text-gray-800 uppercase italic tracking-tighter">Status do Itinerário</h3>
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-widest flex items-center gap-1">
                        <Navigation2 size={12} className="text-[var(--color-primary)]" /> Localização atual: Próximo à Rod. dos Bandeirantes
                    </p>
                </div>
                <Button variant="outline" className="rounded-xl border-gray-100 text-gray-400 hover:text-[var(--color-primary)]">
            ...
            <div className="space-y-8 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100">
                {selectedDriver.paradas.length > 0 ? selectedDriver.paradas.map((parada, idx) => (
                    <div key={idx} className="relative pl-12">
                        ...
                        <div className={cn(
                            ...
                            parada.status === 'concluido' ? "bg-green-500 text-white" :
                            parada.status === 'em_transito' ? "bg-blue-500 text-white animate-pulse" :
                            parada.status === 'atrasado' ? "bg-red-500 text-white" : "bg-gray-200 text-gray-400"
                        )}>
                            {parada.status === 'concluido' ? <CheckCircle2 size={16}/> : 
                             parada.status === 'atrasado' ? <AlertTriangle size={16}/> : <Clock size={16}/>}
                        </div>

                        <div className="flex justify-between items-center">
                            <div>
                                <h5 className="font-black text-gray-800 uppercase italic text-sm leading-none">{parada.local}</h5>
                                <p className="text-[10px] text-gray-400 font-bold uppercase mt-1 tracking-widest">{parada.status.replace('_', ' ')}</p>
                            </div>
                            <div className="text-right">
                                <span className="text-xs font-black text-gray-300 italic">{parada.hora}</span>
                            </div>
                        </div>
                    </div>
                )) : (
                    <div className="flex flex-col items-center justify-center py-20 text-gray-300">
                        ...
                        <p className="font-black uppercase italic text-xs tracking-widest">Aguardando atribuição de rota</p>
                    </div>
                )}
            </div>
