...
const ESTOQUE_MOCK = [
  { id: "LOT-2024-001", material: "Sucata de Gusa", quantidade: 15.5, unidade: "ton", status: "disponivel", ultimaEntrada: "2024-05-20" },
  { id: "LOT-2024-002", material: "Ferro Fundido", quantidade: 8.2, unidade: "ton", status: "reservado", ultimaEntrada: "2024-05-18" },
  { id: "LOT-2024-003", material: "Chapa Recorte", quantidade: 450, unidade: "kg", status: "disponivel", ultimaEntrada: "2024-05-21" },
  { id: "LOT-2024-004", material: "Estamparia", quantidade: 12.0, unidade: "ton", status: "aguardando_triagem", ultimaEntrada: "2024-05-22" },
];
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          <Link to="/registrar-material" className="h-14 px-8 flex items-center bg-white !text-[var(--color-primary)] hover:bg-gray-100 font-black uppercase italic shadow-2xl transition-all border-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "Volume Total", value: "36.4", unit: "Ton", icon: Scale, color: "text-blue-500" },
          { label: "Entradas (Hoje)", value: "+4.2", unit: "Ton", icon: ArrowDownToLine, color: "text-green-500" },
          { label: "Saídas (Hoje)", value: "-1.8", unit: "Ton", icon: ArrowUpFromLine, color: "text-orange-500" },
          { label: "Itens em Triagem", value: "05", unit: "Lotes", icon: History, color: "text-purple-500" },
        ].map((card, i) => (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {['Disponível', 'Reservado', 'Em Triagem'].map((status) => (
                  <label key={status} className="flex items-center gap-3 text-white/80 text-xs cursor-pointer hover:text-white transition-colors p-1">
                    <input type="checkbox" className="rounded border-white/20 bg-transparent text-orange-500 focus:ring-orange-500" />
                    {status}
                  </label>
                ))}
              </div>
                ...
                {ESTOQUE_MOCK.map((item) => (
                  <tr key={item.id} className="group hover:bg-orange-50/30 transition-all">
                    <td className="px-8 py-6">
                      ...
                      <div className="text-[10px] text-gray-400">Entrada: {item.ultimaEntrada}</div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6 text-center">
                      <span className={cn(
                        ...
                        item.status === 'disponivel' ? "bg-green-100 text-green-600" :
                          item.status === 'reservado' ? "bg-blue-100 text-blue-600" : "bg-orange-100 text-orange-600"
                      )}>
